...
according to his or her performance in the class.  Alice wants to give at least 1 
candy to each child. If two children sit next to each other, then the one with the 
higher rating must get more candies. Alice wants to minimize the total number of 
...
Complete the candies function in the editor below. It must return the minimum 
number of candies Alice must buy.
...
Each of the next n lines contains an integer arr[i] indicating 
the rating of the student at position i.
...
const candies = (n, arr) => {
    ...
    for (let i = arr.length - 1; i >= 0; i--) {
        let prev = arr[i]
        let current = arr[i - 1]
        if (prev < current && newArr[i] >= newArr[i - 1]) {
            newArr[i - 1] = newArr[i] + 1
        }
    }
    return newArr.reduce((sum, n) => sum += n, 0)
}
