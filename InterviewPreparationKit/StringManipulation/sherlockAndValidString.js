...
Complete the isValid function in the editor below. It should return either the 
string YES or the string NO.
...
Constraints
1 <= |s| <= 10^5
Each character s[i] is in set ascii[a-z]
...
const isValid = s => {
    ...
    const newArr = Object.values(hash)

    for (let i = 1; i < newArr.length; i++) {
        if (newArr[i] !== newArr[i - 1] && free) {
            delete newArr[i]
            free = false
        }
    }
    console.log(newArr);
    return newArr.every(num => newArr[0] === num) ? 'YES' : 'NO'
}


//cSpell:disable
console.log(isValid('abcc')) //Yes
console.log(isValid('abccc')) //Yes
...
const isValid = s => {
    ...
    for (let i = 1; i < newArr.length; i++) {
        if (newArr[i] !== newArr[i - 1] && free) {
            newArr[i] > newArr[i - 1] ? newArr[i] = newArr[i] - 1 : newArr[i - 1] = newArr[i - 1] - 1
            free = false
        }
    }
    console.log(newArr);
