...
For example, if a = cde and b = dcf, we can delete e from string a and f from string b so that 
both remaining strings are cd and dc which are anagrams.

Function Description
Complete the makeAnagram function in the editor below. It must return an integer representing 
the minimum total characters that must be deleted to make the strings anagrams.
...
const makeAnagram = (a, b) => {
    ...
    for (let letter of a) {
        hashA[letter] = (hashA[letter] || 0) + 1
    }


    for (let key of b) {
        if (hashA[key]) hashA[key] -= 1
        else hashA[key] = (hashA[key] || 0) - 1
    }

    return Object.values(hashA).reduce((sum, n) => sum += Math.abs(n), 0)
}
