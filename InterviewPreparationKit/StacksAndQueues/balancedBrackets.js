...
pair of brackets.Given n strings of brackets, determine whether each sequence of brackets is balanced. 
...
const isBalanced = s => {
    ...
    for (let symbol of s) {
        if (brackets[symbol]) stack.push(symbol)
        else {
            item = stack.pop()
            if (brackets[item] !== symbol) return 'NO'
        }
    }
