...
by a number of sequential preliminary contests. Initially, her luck balance 
...
luck she can have after competing in all the preliminary contests? This value may 
be negative.
...
If Lena loses all of the contests, her luck will be 5 + 1 + 4 = 10. Since she is allowed 
to lose 2 important contests, and there are only 2 important contests. She can lose all 
three contests to maximize her luck at 10. If k = 1, she has to win at least 1 of the 2 
important contests. She would choose to win the lowest value important contest worth 1. 
Her final luck will be 5 + 4 - 1 = 8.

Function Description
Complete the luckBalance function in the editor below. It should return an integer that 
represents the maximum luck balance achievable.
...
Input Format
The first line contains two space-separated integers n and k, the number of preliminary contests 
and the maximum number of important contests Lena can lose.Each of the next n lines contains 
two space-separated integers, L[i] and T[i], the contest's luck balance and its importance rating.
...
const luckBalance = (k, contests) => {
    ...
    for (let i = 0; i < contests.length; i++) {
        if (contests[i][1] === 1) luckPoints.push(contests[i][0])
        if (contests[i][1] === 0) sum += contests[i][0]
    }

    luckPoints.sort((a, b) => b - a)

    winsRequired = luckPoints.length - losses


    for (let i = 0; i < winsRequired; i++) {
        sum -= luckPoints.pop()
    }
    for (let i = 0; i < luckPoints.length; i++) {
        sum += luckPoints[i]
    }
    console.log(luckPoints, sum, winsRequired)
    return sum
}
